...
def decode(
    container,    # If video stream was found, fetch video frames from the video.
    ...
    if backend in ["pyav", "torchvision"]:
        ...
        if num_frames == 32:
            num_frames = 1024#frames._num_frame  # 1024
            #num_frames = int(num_frames)/6
            if num_frames < 64:
                num_frames = 64
            if num_frames>10000:
                num_frames=10000
            ...
            if sampling_rate == 8:
                num_frames = 32
            if num_frames%32 != 0:
                multiplier = math.floor(num_frames/32)
                if multiplier == 0:
                    multiplier = 1
                num_frames = int(multiplier * 32)
        ...
        try:
            index = torch.linspace(start_idx, end_idx, int(num_frames))
        except:
            print(num_frames)
    ...
    '''import cv2
    path = '16_frame_32_seg_clips_xd/'
    np_frames = frames.cpu().detach().numpy()
    h, w = np_frames.shape[1],np_frames.shape[2]
    reduced_path = video_path[28:]#video_path[9:]
    reduced_path = reduced_path[0:len(reduced_path)-4]
    #reduced_path = reduced_path[:len(reduced_path) - 4].split('/')
    #reduced_path = reduced_path[1]
    print(np_frames.shape, reduced_path)

    np_frames = frames.cpu().detach().numpy()
    shape = np_frames.shape
    # Desired chunk size
    chunk_size = (16, shape[1], shape[2], shape[3])
    from pathlib import Path

    # Splitting the array into chunks
    #chunks = np.split(np_frames, int(np_frames.shape[0] // chunk_size[0]), axis=0)
    np_frames= [np_frames[i:i + chunk_size[0], :, :, :] for i in range(0, np_frames.shape[0], chunk_size[0])]
    #out = cv2.VideoWriter(path + reduced_path + "_" + ".mp4", cv2.VideoWriter_fourcc(*'mp4v'), 8,(w, h))
    for ind,seg in enumerate(np_frames):
        file_path = Path(path + reduced_path + "_" + str(ind) + ".mp4")
        if file_path.exists():
            continue
        out = cv2.VideoWriter(path + reduced_path + "_" + str(ind) + ".mp4", cv2.VideoWriter_fourcc(*'mp4v'), 16, (w, h))
